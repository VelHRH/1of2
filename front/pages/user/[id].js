...
const Me = ({ data }) => {
  ...
  useEffect(() => {window.localStorage.getItem('token') && parseJwt(window.localStorage.getItem('token'))._id === data._id ? setAllowEdit(true) : setAllowEdit(false)}, [])
  ...
  <div className="w-full flex">
   ...
   <div className="grid gap-4 grid-cols-5 md:grid-cols-8">
     ...
     <div className="flex-col">
      {favouriteList(data.winners)?.map((winner, index) => (
       <RatingElement
        key={winner._id}
        index={index + 1}
        image={winner.imgUrl}
